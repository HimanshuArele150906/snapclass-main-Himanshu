...
def teacher_tab_take_attendance():   
    ...
    if not subjects:
        st.warning('You havent created any subjects yet! Please create one')
        return
    ...
    with c1:
        if st.button('Clear all photos', width='stretch', type='tertiary', icon='🗑️', disabled= not has_photos):
            st.session_state.attendance_images = []
            st.rerun()

    with c2:
        if st.button('Run Face Analysis', width='stretch', type='secondary', icon='📈', disabled=not has_photos):
            with st.spinner('Deep scanning classroom photos...'):
                ...
                for idx, img in enumerate(st.session_state.attendance_images):
                    ...
                    if not enrolled_student:
                        st.warning('No studennts enrolled in this course')
                    else:
    ...
    with c3:
        if st.button('Use Voice Attendance', type='primary', width='stretch', icon='🎙️'):
...
# List all sub
    ...
    if subjects:
        for sub in subjects:
            stats = [
                ("👥", "Students", sub.get('total_students') or 0),
                ("🕛", "Classes", sub['total_classes'])

            ]
        def share_btn():
            if st.button(f"Share Code: {sub['name']}", key=f"share{sub['subject_code']}", icon='😀'):
...
def  teacher_tab_attendance_records():   
    ...
    for r in records:
        ...
        data.append({
            ...
            "Subjects": r['subjects']['subject_code'],
    ...
    summary = (
        df.groupby(['ts_group', 'Time', 'Subjects'])
        .agg(
    ...
    display_df = (summary.sort_values(by='ts_group', ascending=False)
                  [['Time', 'Subjects',  'Attendance Stats']]
                  )
...
def teacher_screen_login():
    ...
    teacher_username = st.text_input("Enter username", placeholder='ananhd')
    ...
    with btnc2:
        if st.button('Register Instead',type="primary", icon='🔑', width='stretch'):
...
def teacher_screen_register():
    ...
    teacher_name = st.text_input("Enter your name", placeholder='abcd')
    ...
    with btnc1:
        if st.button('Register now', icon='🔑', shortcut="control+enter", width='stretch'):
    ...
    with btnc2:
        if st.button('Login Instead', type="primary", icon='🔑', width='stretch'):
            st.session_state.teacher_login_type = 'login'
    footer_dashboard()
